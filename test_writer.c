...
int main(){
    ...
    srsvm_word static_program[] = {
        ...
        (OPCODE_MK_ARGC(2) | 9), 0, 5,
        (OPCODE_MK_ARGC(1) | 20), 0,
    ...
    c->const_val.ptr_offset = 13 * (srsvm_word) sizeof(srsvm_word);
    c->const_slot = 3;

    c->next = srsvm_program_const_alloc();
    c = c->next;
    c->const_val.type = PTR_OFFSET;
    c->const_val.ptr_offset = -22 * (srsvm_word) sizeof(srsvm_word);
    c->const_slot = 4;
    
    c->next = srsvm_program_const_alloc();
    c = c->next;
    c->const_val.type = WORD;
    c->const_val.word = 1000;
    c->const_slot = 5;
    
    program->num_constants = 6;

    program->constants_compressed = true;
