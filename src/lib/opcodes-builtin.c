...
void builtin_ARGC(srsvm_vm *vm, srsvm_thread *thread, const srsvm_word argc, const srsvm_word argv[])
{
	srsvm_register *dest_reg = register_lookup(vm, thread, argv[0]);
...
void builtin_ARGV(srsvm_vm *vm, srsvm_thread *thread, const srsvm_word argc, const srsvm_word argv[])
{
	srsvm_register *dest_reg = register_lookup(vm, thread, argv[0]);
	srsvm_register *arg_index_reg = register_lookup(vm, thread, argv[1]);

	if(dest_reg != NULL && arg_index_reg != NULL && !fault_on_not_writable(thread, dest_reg)){
		srsvm_word arg_index;

		if(! reg_read_word(arg_index_reg, &arg_index, 0)){
			dbg_puts("failed to read word from register");
		} else if(arg_index >= vm->argc){
			thread->has_fault = true;
			thread->fault_str = "Attempt to read past argv end";
