...
#define CAST_CONVERTER(name,type_from,ctype_from,field_from,type_to,ctype_to,field_to) \
    ...
    { \
        ...
        srsvm_word dest_offset = argc < 3 ? 0 : argv[2]; \
        srsvm_word src_offset = argc < 4 ? 0 : argv[3]; \
...
#define TOSTR_CONVERTER(name,type_from,ctype_from,field_from,fmt) \
    ...
    { \
        ...
        srsvm_register *dest_reg = register_lookup(vm, thread, argv[0]); \
        srsvm_register *src_reg = register_lookup(vm, thread, argv[1]); \
        srsvm_word src_offset = argc < 3 ? 0 : argv[2]; \
        if(dest_reg != NULL && src_reg != NULL && !fault_on_not_writable(thread, dest_reg)){ \
...
#if WORD_SIZE == 128
void conversion_PARSE_U128_128(srsvm_vm *vm, srsvm_thread *thread, const srsvm_word argc, const srsvm_word argv[])
{
    srsvm_register *dest_reg = register_lookup(vm, thread, argv[0]);
    srsvm_register *src_reg = register_lookup(vm, thread, argv[1]);
    srsvm_word dest_offset = argc < 3 ? 0 : argv[2];
...
void conversion_U128_TO_STR_128(srsvm_vm *vm, srsvm_thread *thread, const srsvm_word argc, const srsvm_word argv[])
{
    static char buf[1024];
    srsvm_register *dest_reg = register_lookup(vm, thread, argv[0]);
    srsvm_register *src_reg = register_lookup(vm, thread, argv[1]);
